...
"""
  if xmax > padsize // 2:
    ...
    notcut, file, offset = in1, in2,(padsize-xmax)/fs
  else:
    if show: show2(fs,s1[xmax:],s2,title='1st=blue=cut;2nd=red (%s;%s)'%(in1,in2))
    notcut, file, offset = in2, in1, xmax/fs
  print(sync_text%(file,offset))
  os.system('ffmpeg -ss {} -i {} output_{}'.format(offset, file, file))
  #os.system('ffmpeg -i {} -i output_{} -filter_complex hstack=inputs=2:shortest=1 Stack.MOV'.format(notcut, file))
